...
\usepackage{booktabs}
\usepackage{tikz}
\usetikzlibrary{arrows.meta,positioning,patterns,shapes,calc,decorations.pathreplacing}
\usepackage{hyperref}
...
\begin{frame}{Classification des variables qualitatives expliquées}
    \begin{center}
        ...
        \node[box] (dicho) at (-6,-1.5) {Dichotomiques};
        \node[box] (poly) at (-2,-1.5) {Polytomiques ordonnées};
        \node[box] (count) at (2,-1.5) {Comptage};
        \node[box] (cens) at (6,-1.5) {Censurées/Tronquées};
...
\begin{frame}{Objectif économétrique}
    ...
    \begin{block}{Idée}
        On va construire un \textbf{modèle latent} (inobservable) qui représente le critère de décision sous-jacent.
    \end{block}
\end{frame}


\begin{frame}{Le cadre : variable dépendante binaire}

\textbf{Objectif :} Modéliser la probabilité qu'un événement se réalise.

\medskip

\textbf{Exemples :}
\begin{itemize}
    \item Participer au marché du travail ($Y_i = 1$) ou non ($Y_i = 0$)
    \item Acheter un produit, faire défaut sur un crédit, voter pour un candidat...
\end{itemize}

\bigskip

\textbf{Modèle de Probabilité Linéaire (MPL) :}
\[
...
\textcolor{red}{\textbf{Question :}} Peut-on estimer $\beta$ par MCO comme dans le modèle linéaire standard ?

\end{frame}

%==============================================================================
\section{Problème 1 : Hétéroscédasticité structurelle}
%==============================================================================

\begin{frame}{Problème 1 : Hétéroscédasticité structurelle}

$Y_i | X_i$ suit une loi de \textbf{Bernoulli} de paramètre $p_i = X_i'\beta$.
...
$\Rightarrow$ La variance \textbf{dépend de $X_i$} : hétéroscédasticité \textbf{inhérente} au modèle.

\end{frame}

\begin{frame}{Conséquences de l'hétéroscédasticité}

\textbf{Rappel :} Sous hétéroscédasticité, l'estimateur MCO reste sans biais et convergent, mais :

\bigskip

\begin{enumerate}
    \item \textbf{Inefficacité :} $\hat{\beta}_{MCO}$ n'est plus BLUE (Best Linear Unbiased Estimator)

    \bigskip

    \item \textbf{Inférence invalide :} La matrice de variance usuelle
    \[
    \widehat{\text{Var}}(\hat{\beta}) = \hat{\sigma}^2 (X'X)^{-1}
    \]
    est \textbf{incorrecte}. Les tests $t$ et $F$ ne sont plus valides.
...
\textcolor{orange}{\textbf{Mais cela ne résout pas les autres problèmes...}}

\end{frame}

%==============================================================================
\section{Problème 2 : Prédictions hors de [0,1]}
%==============================================================================

\begin{frame}{Problème 2 : Prédictions hors de $[0,1]$}
...
$\Rightarrow$ Des \textbf{probabilités négatives} ou \textbf{supérieures à 1} : absurde !

\end{frame}

%==============================================================================
\section{Problème 3 : Non-normalité des résidus}
%==============================================================================

\begin{frame}{Problème 3 : Non-normalité des résidus}
...
$\Rightarrow$ Distribution \textbf{bimodale}, clairement \textbf{non-gaussienne}.

\smallskip

$\Rightarrow$ Les tests $t$ et $F$ (basés sur la normalité) sont invalides en \textbf{échantillon fini}.

\end{frame}

%==============================================================================
\section{Problème 4 : Effets marginaux constants}
%==============================================================================

\begin{frame}{Problème 4 : Effets marginaux constants (irréalistes)}
...
$\Rightarrow$ L'effet d'une variable devrait \textbf{s'atténuer aux extrêmes} (saturation).

\bigskip

\textcolor{blue}{\textbf{Intuition :}} Augmenter le revenu de quelqu'un qui a déjà 99\% de chances d'acheter une maison n'aura presque aucun effet sur sa probabilité d'achat.

\end{frame}

\begin{frame}{Comparaison des effets marginaux}
...
\textbf{Modèles non-linéaires :} L'effet marginal est maximal au centre ($P \approx 0.5$) et tend vers 0 aux extrêmes $\Rightarrow$ \textbf{effet de saturation} réaliste.

\end{frame}

%==============================================================================
\section{Récapitulatif et solution}
%==============================================================================

\begin{frame}{Récapitulatif : limites du MPL}
...
\hline
Hétéroscédasticité structurelle & Inférence invalide & Moyenne \\
& (corrigeable par HC) & \\
\hline
Prédictions hors $[0,1]$ & Probabilités absurdes & \textcolor{red}{Élevée} \\
\hline
Résidus non-normaux & Tests invalides & Moyenne \\
& (asymptotiquement OK) & \\
\hline
Effets marginaux constants & Modèle mal spécifié & \textcolor{red}{Élevée} \\
\hline
\end{tabular}
\end{center}
...
\textbf{Conclusion :} Le MPL peut être utilisé comme \textbf{approximation rapide} pour des probabilités proches de 0.5, mais il est \textbf{inadapté} pour une modélisation rigoureuse.

\end{frame}

\begin{frame}{Solution : les modèles à variable latente}

\textbf{Idée :} Introduire une variable latente $Y_i^*$ continue :
\[
...
\begin{frame}{Comparaison graphique : MPL vs Probit/Logit}
...
$\Rightarrow$ Les modèles Probit et Logit garantissent $P \in [0,1]$ et capturent la \textbf{saturation} aux extrêmes.
...
\begin{itemize}
    \item[\textcolor{green!60!black}{\checkmark}] Simple à estimer et interpréter
    \item[\textcolor{green!60!black}{\checkmark}] Peut servir d'approximation locale
    \item[\textcolor{red}{$\times$}] Hétéroscédasticité structurelle
    \item[\textcolor{red}{$\times$}] Prédictions hors de $[0,1]$
    \item[\textcolor{red}{$\times$}] Effets marginaux constants (irréalistes)
\end{itemize}
...
\begin{itemize}
    \item \textbf{Probit} : $P(Y=1|X) = \Phi(X'\beta)$
    \item \textbf{Logit} : $P(Y=1|X) = \Lambda(X'\beta) = \frac{e^{X'\beta}}{1+e^{X'\beta}}$
    \item Nous pourrions, en principe, considérer d'autre distributions
    \end{itemize}

\bigskip

$\Rightarrow$ Estimation par \textbf{maximum de vraisemblance}, propriétés asymptotiques bien établies.
...
\begin{frame}{Le cas d'une variable polytomique ordonnée}
    \begin{definitionfr}
...
\begin{frame}{Le cas d'une variable de comptage}
    \begin{definitionfr}
