...
\item L'\textbf{effet incrémental} de $X$ sur la probabilité est la
  différence de probabilités quand $X$ passe de 0 à 1 :
  \[
  \Delta_X = \PP(Y=1 \mid X=1) - \PP(Y=1 \mid X=0) = F(\beta_0 + \beta_1) - F(\beta_0)
  \]
...
\begin{frame}{Interprétation -- Élasticité}

\begin{itemize}

\item Pour une variable \textbf{continue} $X_k$, l'\textbf{élasticité} au
  point $X$ mesure la variation relative de $\PP(Y=1)$ pour une variation
  de 1\% de $X_k$ :
  \[
  e_{X_k} = \frac{\partial \ln \PP(Y=1 \mid X)}{\partial \ln X_k} = \beta_k \times f(X\beta) \times \frac{X_k}{F(X\beta)}
  \]

\item L'élasticité permet de \textbf{comparer} l'influence de variables
  mesurées sur des échelles différentes (euros, années, heures,
  etc.).\newline

\item Comme l'effet marginal, l'élasticité varie avec $X$. On l'évalue
  généralement au \textbf{point moyen}.
...
L'effet marginal (dérivée) n'est défini que pour les variables
\textbf{continues}. Pour les variables binaires, on utilise l'\textbf{effet
incrémental}.

\end{frame}

\begin{frame}{Application 1 -- Élasticités au point moyen}
...
\item Une hausse de 1\% de la limite de crédit réduit la probabilité de
  défaut de \textbf{0,34\%}.\newline

\item L'âge a une influence beaucoup plus faible : $+0.15$\% pour une
  hausse de 1\%.
...
\begin{frame}{Application 2 -- Distribution des heures travaillées}
...
\begin{center}
    \begin{tikzpicture}
        \begin{axis}[
            ...
            width=12cm, height=5.5cm,
            ylabel={Proportion (\%)},
            symbolic x coords={1--10, 11--20, 21--30, 31--39, 40, 41--45, 46--50, 51--60, 61--99},
            xtick=data,
            xlabel={Heures par semaine},
            ymin=0, ymax=55,
            nodes near coords={\pgfmathprintnumber\pgfplotspointmeta\%},
            nodes near coords align={vertical},
            every node near coord/.append style={font=\small},
        ...
        \addplot coordinates {
            (1--10, 1.7) (11--20, 6.2) (21--30, 6.6) (31--39, 7.7)
            (40, 47.2) (41--45, 7.8) (46--50, 11.2) (51--60, 8.0) (61--99, 3.5)
        };
        \end{axis}
    \end{tikzpicture}
\end{center}

\textbf{47\%} des individus déclarent exactement 40 heures (semaine légale
standard aux É.-U.). La variable n'est pas véritablement continue : l'effet
marginal et l'élasticité ont ici un intérêt limité. L'effet
\textbf{interdécile} (25h $\to$ 55h) est plus informatif.
...
\begin{frame}{Application 2 -- Élasticités au point moyen}
...
\item Les deux variables continues ont des élasticités proches de
  \textbf{1} : une hausse de 1\% de l'âge ou des heures travaillées
  augmente la probabilité de revenu élevé d'environ 1\%.\newline

\item Les élasticités permettent de comparer des variables sur des
  échelles différentes : ici l'âge et les heures ont un effet
  \textbf{comparable}.
...
\item Cameron, A.C. \& Trivedi, P.K. (2005). \textit{Microeconometrics: Methods
  and Applications}, Cambridge University Press, ch.~16.
